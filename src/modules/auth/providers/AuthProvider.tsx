...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  const login = async (auth: AuthLoginDto) => {
    try {
      const authUserResult = await handleLogin(auth);
      ...
      const authUserData = await chooseAccountAndProfileAndConvertToUser();
  ...
  const chooseAccountAndProfileAndConvertToUser = async () => {
    try {
      const authAccount = await handleGetAuthAccount();

      const authProfile = await handleGetAuthProfile(authAccount.id);

      return UserMapper.accountAndProfileToUser(authAccount, authProfile);
    } catch (err) {
      toast('Não foi possível capturar perfil', { type: 'error' });
      throw err;
    }
  };
